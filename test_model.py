from model import Game, PlayingLineup, ShotStat, ShotHandle
...
def test_add_game_player_lineup():
    ...
    positions = PlayingLineup('nico', 'edu', 'mikel', 'sergio', 'sergio', 'nico', 'luis')
...
def test_create_shot_stat():
    shot = ShotStat(1, 1, 'draw', ShotHandle.in_turn, 1, 4, 'draw')

    assert shot.end == 1
    assert shot.throw_in_end == 1
    assert shot.thrower_position == 'lead'
    assert shot.shot_type == 'draw'
    assert shot.handle.value == "IN"
    assert shot.difficulty == 1
    assert shot.score == 4
    assert shot.draw_or_hit == 'draw'

    shot = ShotStat(1, 2, 'draw', ShotHandle.out_turn, 1, 4, 'draw')
    assert shot.thrower_position == 'lead'
    assert shot.handle.value == "OUT"

    shot = ShotStat(1, 3, 'draw', ShotHandle.out_turn, 1, 4, 'draw')
    assert shot.thrower_position == 'second'

    shot = ShotStat(1, 5, 'draw', ShotHandle.out_turn, 1, 4, 'draw')
    assert shot.thrower_position == 'third'

    shot = ShotStat(1, 7, 'draw', ShotHandle.out_turn, 1, 4, 'draw')
...
#     positions = PlayingLineup('nico', 'edu', 'mikel', 'sergio', 'sergio', 'nico', 'luis')
#
#     shot1 = ShotStat(1, 1, 'draw', ShotHandle.out_turn, 1, 4, 'draw')
#     shot2 = ShotStat(1, 2, 'draw', ShotHandle.in_turn, 2, 4, 'draw')
#     shot3 = ShotStat(1, 3, 'hit', ShotHandle.out_turn, 2, 4, 'hit')
#     shot4 = ShotStat(1, 4, 'draw', ShotHandle.in_turn, 3, 4, 'hit')
#
